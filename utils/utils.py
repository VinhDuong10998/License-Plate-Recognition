...
# Plotting functions ---------------------------------------------------------------------------------------------------
def plot_one_box(x, img, color=None, label=None, line_thickness=None,refine_net=None,net=None,text_model=None):
    ...
    recognized = ''
    print(label.split())
    if float(label.split()[-1]) > 0.75:
        bsx = img[int(x[1]):int(x[3]),int(x[0]):int(x[2])]
        # cv2.imwrite('input/detection_{}_{}.jpg'.format(int(x[0]),int(x[1])),img)
        cv2.imwrite('input/detection_{}_{}.jpg'.format(int(x[0]),int(x[1])),bsx)
        recognized = craft_model.run_model(net,refine_net,bsx,"detection_{}_{}".format(int(x[0]),int(x[1])),text_model)
    # cv2.imshow("img_{}".format(color),bsx)
    # cv2.waitKey(0)
    cv2.rectangle(bb, c1, c2, color, thickness=tl)
    test_folder = 'input/'
    if label:
        tf = max(tl - 1, 1)  # font thickness
        t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        cv2.rectangle(bb, c1, c2, color, -1)  # filled
        if recognized != '':
           cv2.putText(bb, recognized, (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
    return bb, lt, rb
